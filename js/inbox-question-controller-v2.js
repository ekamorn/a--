$(document).ready(function() {
	...
	CallService(post_inbox, main_link + 'web_client_service/inbox_outbox_client_service.php', false, function(msg) {
		...
		$('.inbox-question').click(function(e) {
			...
			$('.inbox-question').each(function() {

				if ($(this).hasClass('active') && inboxLength > 0) {

					createInboxLinks(inboxLength);
		...
		CallService(post_inbox, main_link + 'web_client_service/inbox_outbox_client_service.php', false, function(msg) {
			...
			$('.inbox-question').each(function() {
				...
				if ($(this).hasClass('active')) {
					...
					$('.content-collection').remove();
					createInboxLinks(inboxLength);
					createInboxInfoQuestion(target_inbox);
	...
	function createBigInboxLinks (data) {

		allInbox = data;
		inboxLength = allInbox.length;
	...
	function createInboxLinks (dataLength) {
		...
		for (var i = 0; i < dataLength; i++) {
			...
			$('.question').each(function() {
				...
				if ($(this).attr('questionId') == idQuestion) {

					$('.question:first').removeClass('active');
					$(this).addClass('active');
				}
			});
			...
			$('.inboxx-question').click(function(e) {
				...
				idQuestion = $(this).attr('questionId');
				...
				$(this).addClass('active');
				$('.content-collection').remove();
				target_inbox = $(this).attr('targetValue');
				createInboxInfoQuestion(target_inbox);
	...
	function createInboxInfoQuestion (length) {
		...
		// getDate
			...
			var previousYear   = currentYear - year;
			var previousMonth  = currentMonth - month;
			var previousDay    = cD - d;
			var previousHour   = cH - h;
			var previousMinute = cM - m;


			if (previousYear >= 1 && previousMonth >= 0) {

				var pastTime = previousYear + " ปีก่อน";
			
			} else if (previousYear >= 1 && previousMonth < 0) {

				var pastTime = previousMonth + 12 + " เดือนก่อน";
			
			} else if (previousYear == 0 && previousMonth == 1 && previousDay < 0) {

				var pastTime = previousDay + 30 + " วันก่อน";

			} else if (previousYear == 0 && previousMonth >= 1) {

				var pastTime = previousMonth + " เดือนก่อน";
			
			}  else if (previousYear == 0 && previousMonth == 0 && previousDay >= 1) {

				var pastTime = previousDay + " วันก่อน";

			} else if (previousYear == 0 && previousMonth == 0 && previousDay == 0 && previousHour == 0 && previousMinute >= 0) {

				var pastTime = cM - m + " นาทีก่อน";

			} else if (previousYear == 0 && previousMonth == 0 && previousDay == 0 && previousHour >= 0 && previousHour < 1 && previousMinute > 0) {

				var pastTime = cM - m + " นาทีก่อน";

			} else if (previousYear == 0 && previousMonth == 0 && previousDay == 0 && previousHour > 0) {
	...
	// ---------------------------------------------- Create Outbox Links function -------------------------------------------------

	function createBigLinks (data) {

		allOutbox = data;
		outboxLength = allOutbox.length;
		console.log(outboxLength);
	...
	function createLinks (dataLength) {
		...
		for (var i = 0; i < dataLength; i++) {
			...
			$('.question').click(function(e) {
				...
				$('.content-collection').remove();
				$('.answer-collection').remove();
				target_question = $(this).attr('targetValue');
				createInfoQuestion(target_question);
			...
			if (answerId != 0 && isReadStatus == false && questionStatus == 1) {

				$('.question').click(function(e) {

					var qqId = $(this).attr('questionId');
					var isRead = true;
					var isReadJson = {
														"IsRead1" : isRead,
														"questionId1" : qqId
